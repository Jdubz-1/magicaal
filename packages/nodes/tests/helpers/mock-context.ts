...
export function makeMockContext(initial: Record<string, unknown> = {}): MockContext {
  ...
  const ctx: MockContext = {
    ...
    evaluate: jest.fn().mockImplementation(async (expr: string) => {
      // basic passthrough — tests that need real evaluation should use the jsonata util directly
      return expr;
    }),

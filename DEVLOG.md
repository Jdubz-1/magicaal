...
**Description:**
Following the docs/ rewrite (previous entry), a further gap remained specifically for Graph-as-Code (`agents/*.agent.ts`) authoring: there was no single reference an AI coding agent (or a human) could work from without reading `packages/compiler` and all 53 node source files directly. Worse, the one thing that looked like such a reference — `packages/compiler/src/node-classes.ts`'s typed helper classes, generated by a `codegen` step — turned out to be stale in ways that would produce broken agents if trusted: `LlmCallNode`'s config is missing the actively-used `userMessageKey` field, `RouterNode` claims a `routes` field that was never real (the real field is `cases`), and `EndNode` claims a singular `outputKey` where the real field is the plural `outputKeys`. All three were found by reading the real node source in `packages/nodes/src/nodes/` and comparing.

`docs/developer-guide/AGENTS.md` is a self-contained instruction set for generating `*.agent.ts` files: the full `AgentGraph` API (`node`/`connect`/`when`/`otherwise`/`tool`/`workspace`), the complete `AgentConfig`/`SessionConfig` shapes, the graph-validity rules `compile()` actually enforces (vs. what it doesn't — node config shape isn't validated at compile time), JSONata/prompt-building conventions, and a pitfalls checklist built from real bugs documented inline in `agents/caal.agent.ts`. `docs/reference/nodes/` backs it with one verified page per `core:*` node type — config table, output keys, behavior notes, and a usage example — written directly from each node's `interface <X>Config`, JSON Schema, and `execute()` body, not from the stale typed helpers.

**Changes:**
- New: `docs/developer-guide/AGENTS.md`
- New: `docs/reference/nodes/README.md` + 53 `core-<type>.md` pages, organized by each node's real `meta.category` (10 categories, verified against `packages/sdk/src/node.ts`)
- Rewrite: `docs/reference/node-catalog.md` — regrouped to match the same real category taxonomy (previously used an invented 6-category grouping that didn't match the code); every node name now links to its reference page
- New: `docs/developer-guide/graph-as-code.md`, `docs/developer-guide/node-authoring.md` — cross-linked to the new pages; `graph-as-code.md`'s compiler module table also corrected (`node-classes.ts` was mislabeled as the `AgentGraph` base class, which actually lives in `graph.ts`)

**Impact:**
Graph-as-Code authoring — by a human or an AI agent — now has a config reference that's been verified against source rather than against the drifted typed helpers, closing a class of bug (wrong field names compiling fine and failing/misbehaving at runtime, since `compile()` doesn't validate node config shape) before it happens rather than after.

---

### 2026-08-03 - Rewrote docs/ and DEVELOPMENT.md to match the current monorepo

**Type:** Documentation

**Description:**
An audit of `docs/` (`.ai_docs/MAGICAAL_DOCS_AUDIT_REPORT.md`, gitignored) found it was an unmodified generic scaffold left over from an unrelated starter template: it described a single Express service (`npm install`, port 3000 only, a nonexistent `packages/types/` layout, Node ~20) with almost no relation to the real 3-app/9-package/16-integration-package MagiCaal monorepo. `DEVELOPMENT.md` at the repo root had the same problem and was linked from `docs/` as an authoritative entry point, so it was included in the fix rather than left contradicting the rewritten pages. Every existing `docs/` page had at least one actively wrong instruction — wrong package manager, wrong Node version, wrong service topology, and a contributing guide that told contributors to PR against `main` and said nothing about the DCO sign-off `CONTRIBUTING.md` requires — and 21 real subsystems (the engine, the web app, most of `packages/*`, Marketplace, MCP, RBAC, sessions, Caal, the Model Router) had zero coverage anywhere in `docs/`.

The rewrite closed this in three tiers: fixing every broken command/path/version and aligning branch/DCO guidance with `CONTRIBUTING.md`; adding architecture and subsystem pages for everything that was undocumented, plus filling in the ~50 path entries missing from `apps/api/src/openapi/spec.ts` across 9 controllers (`api-keys`, `caal`, `datasources`, `mcp-servers`, `prompts`, `runs`, `tenants`, `test-cases`, `users`) since the API reference page now points at the live `GET /v1/openapi.json` instead of hand-duplicating endpoints; and rounding out reference pages for the remaining packages, the CLI, the audit log, and a Docker Compose quickstart.

**Changes:**
- `DEVELOPMENT.md` — full rewrite for the real monorepo layout, Devbox+pnpm commands, and the 3-Dockerfile build process
- `apps/api/src/openapi/spec.ts` — added the missing path entries noted above
- `docs/developer-guide/{README,api/README,architecture/README,contributing/README,contributing/code-standards}.md`, `docs/getting-started/{README,installation}.md`, `docs/reference/README.md` — corrected in place
- New: `docs/developer-guide/architecture/{engine,web,model-router}.md`, `docs/developer-guide/{auth-rbac,sessions,database,mcp,marketplace,caal}.md`, `docs/developer-guide/contributing/rfc-process.md`, `docs/getting-started/docker-quickstart.md`, `docs/reference/{core-types,sdk-client,cli,audit-log}.md`, `docs/reference/integrations/README.md`
- `CLAUDE.md`, `README.md` — corrected the Node.js version reference (22 → 24 LTS, matching `devbox.json`) found during the same audit

**Impact:**
Every command in `docs/` is now copy-paste-runnable against the real repo, and `docs/` no longer contradicts `CLAUDE.md`/`CONTRIBUTING.md`/`README.md` on branch strategy, DCO, Node version, or service topology. `docs/` grew from 8 files to a fully covered set; the audit report remains as the traceable record of what was found and fixed.
...
**Description:**
A completeness review of Caal (the Studio AI assistant) found several documented capabilities missing or broken (a broken History panel, dropped `delete_edge` proposal patches, admin config never read at invocation, no SSE despite the docs claiming one). Re-verifying each fix against the real node modules — rather than by hand-evaluating JSONata, which is what let this ship unnoticed for over a month — surfaced something far more fundamental: `agents/caal.agent.ts` had six separate nodes configured with fields their node type doesn't recognize, two of which were fatal. `intent-router` used `routeKey` instead of `core:router`'s real `expression`/`cases` config, so `jsonata(undefined)` threw at construction and failed *every single Caal invocation* before any response path ever ran. Even past that, every `core:transform` node (`context-assembler`, `response-assembler`) never set `outputKey`, and `core:transform` writes its JSONata result to exactly one context key rather than spreading an object result's fields across the top level — so none of `context-assembler`'s fields (`intent`, `graphContext`, the per-intent message strings) ever reached any downstream node, and the final response `content` never reached the user. Caal has, in all likelihood, never produced a working response since it shipped in Phase 4.

**Changes:**
- `agents/caal.agent.ts` — replaced the single `context-assembler` node (one big JSONata object literal, no `outputKey`, so it landed entirely under a bogus key) with several single-purpose `core:transform` nodes: `assemble-graph-context`, `assemble-system-context` (upstream of routing, since routing itself depends on `isCodeDefined`), and per-branch `build-explain-message`/`build-suggest-message`/`build-modify-message` (only the relevant one runs per invocation). `intent-router` now uses a valid `expression`/`cases` config instead of the crashing `routeKey`. `explainer`/`suggester`/`modifier` now read their message from a real context key (`userMessageKey`/`inputKey`) instead of a dead `userTemplate` field, and all write their answer to `outputKey: 'content'`. `response-assembler`'s remaining multi-field result (`proposal`/`canvasHighlight`/`canvasFocus`/`nodeReferences`) is nested under `caalResult` and flattened back to the top level by `invokeCaal`, since `core:transform` can't produce multiple sibling keys. Dead `routerConfig`/`concurrency` fields (also wrong shapes for their real types) removed/fixed; Caal's LLM calls now fall through to the graph/tenant router policy, steerable per-tenant via a new `routerOverride`.
- `packages/nodes/src/nodes/core-llm-call.ts` — new `userMessageKey` config field (reads a pre-assembled string from context, parallel to the existing `messagesKey`), and both this node and `apps/engine/src/execution/tool-executor.ts`'s `runAgentLoop` now append a `systemPromptSuffix` read from context onto the configured system prompt.
- `apps/api/src/controllers/caal.controller.ts` — resolves the calling tenant's `caal_configuration` before dispatch: `enabled: false` now actually blocks invocation (403 `CAAL_DISABLED`); `routerPolicyId` resolves to a `routerOverride` threaded through `POST /internal/runs` → the BullMQ job → `ExecutionContextImpl` → `resolveRouterConfig()` (new `apps/engine/src/router/router-engine.ts` precedence tier, ranked below a locked tenant policy but above node/graph config); `systemPromptSuffix` passes through as run input. `modelOverride` is a documented limitation (a bare model name has no provider/connection to build a route from). The 30s poll-for-completion ceiling (`invokeCaal` dispatches to the engine and polls within the request — there is no SSE, despite both the roadmap and PROGRESS.md claiming one) is now a configurable `CAAL_INVOKE_TIMEOUT_MS` (default 120s), and a run that outlasts it returns a distinct `202 CAAL_STILL_RUNNING` instead of a bare `504`, since the run may still complete server-side.
- `apps/web/src/canvas/components/CaalPanel.svelte` — fixed the History panel (it called `/api/v1/caal/sessions/:id` with the already-namespaced session ID instead of the plain agent ID the controller expects, and never sent `agentId` in the invoke body at all, so every agent a user edited shared one `global` Caal session); added the missing `agentId` to the invoke body; added a context bar (node count / selected node / last run status) and `lastRunResult` wiring, both previously dead; added an "Undo last Caal change" action; shows a distinct message on `CAAL_STILL_RUNNING`.
- `apps/web/src/canvas/App.svelte` — `handleApplyProposal` now handles `delete_edge` patches (silently dropped before) and snapshots the graph via the new `apps/web/src/canvas/stores/caalUndo.ts` store before applying, enabling the undo action above. Modeled directly on the pre-existing `autoPlacementUndo.ts` single-slot pattern rather than building a general undo/redo stack (none exists anywhere in Studio).
- `apps/web/src/routes/admin.ts` — the Admin Caal Configuration page called `/v1/system/caal-config`, a route that has never existed at any point; it always showed defaults on load and threw on every save. Fixed to call the real `/v1/caal/config`. Added the missing `routerPolicyId` selector (fetched from `/v1/llm/router-policies`); disabled and labeled `modelOverride`/`generationMode`/`confirmationMode`/`showReasoning` as not-yet-implemented rather than letting them imply they do something.
- TypeScript-diff suggestions for code-defined agents (never implemented past an unused type) are explicitly descoped: the modify path now routes code-defined agents to a fixed "not supported yet, edit the source and run `magicaal build`" response instead of silently misbehaving.
- New regression tests: `packages/cli/tests/unit/caal-agent-compile.test.ts` compiles the real `CaalAssistantAgent` and asserts every node's config shape matches what its node type actually reads — this is the test that would have caught the whole class of bug immediately had it existed from the start. Also new: `apps/engine/tests/unit/execution/tool-executor.test.ts` (`runAgentLoop` had zero tests before this), `resolveRouterConfig` precedence coverage in `apps/engine/tests/unit/router/router-engine.test.ts`, and `apps/api/tests/integration/caal-still-running.test.ts`.

**Impact:**
Caal should now actually work end-to-end for the first time — explain, suggest, and modify all deliver the real user message and graph context to the LLM, and the LLM's response actually reaches the user and gets persisted to session history. Tenant admins can steer Caal's model routing and disable it entirely, both previously silent no-ops. 149 nodes + 276 engine + 361 API + 21 CLI + 12 web tests green; `apps/web` canvas build and type-checks across `api`/`engine`/`web` clean.

**Notes:**
Full findings and fix-by-fix detail (19 issues, ISS-067 through ISS-085) are logged in the gitignored `.ai_docs/MAGICAAL_ISSUES.md` working doc, not in git history — that file predates this repo's `.ai_docs`-untracking decision and is treated as a local review/planning artifact, consistent with how it's been used in prior entries in this log. `modelOverride` and four other `caal_configuration` fields (`generationMode`, `confirmationMode`, `allowedOperations`, `showReasoning`) remain intentionally unimplemented — labeled as such in the admin UI rather than built, since each implies a real behavior change (auto-apply mode, tool allowlisting, reasoning display) worth scoping separately.
